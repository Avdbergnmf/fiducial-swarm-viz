    ...
    public sealed class CueOverlay : MonoBehaviour, IRunView
    {
        ...
        public static readonly CueSpec[] Specs =
        {
            ...
            new(CueMask.Cover, "Kill envelope",
                "Theoretical safety belt around the asset. Each living friendly paints the inbound directions it could still ram, assuming a hostile that first appears on its sense sphere and flies params maxv straight at the asset. Divert is Reach around that drone's ballistic point (pose + velocity × t) — an orbiting picket is already moving, a parked one is the old from-rest belt. Green is covered; red is a hole. Closed means the picket-height ring has no gap.",
                "Live pose and velocity from the trace; sense / maxv / lat / ring from the params log; kill_radius and asset.radius from the header. Reach() as in flight.cpp, divert = lat. Not UniqueOwner, not scramble delay, not the 0.1 s cylinder gate — geometry only. Hostile speed is maxv (D37), not an unpublished enemy dump. Brain radius shrink is still from rest (D58), so this belt can be greener than the planned ring."),
